...
def main(model, weights, env, n_episodes=3, max_steps=int(1e9), show=False, record=False, video_dir="./video"):
    ...
    agent = MineRLAgent(env, device="cpu", policy_kwargs=policy_kwargs, pi_head_kwargs=pi_head_kwargs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weights", type=str, required=True, help="Path to the '.weights' file to be loaded.", default="data/VPT-models/foundation-model-1x.weights")
    parser.add_argument("--model", type=str, required=True, help="Path to the '.model' file to be loaded.", default="data/VPT-models/foundation-model-1x.model")
    ...
    main(args.model, args.weights, args.env, show=args.show or show, record=args.record, max_steps=args.max_steps, video_dir=args.video_dir)
