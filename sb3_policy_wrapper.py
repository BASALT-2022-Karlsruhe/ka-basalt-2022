...
class MinecraftActorCriticPolicy(ActorCriticPolicy):
    ...
    def forward(self, obs: th.Tensor, first: th.Tensor, state_in, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        (pi_logits, vpred, _), state_out = self.minerl_agent.policy(obs, first, state_in)
        action = self.minerl_agent.policy.pi_head.sample(pi_logits, deterministic=deterministic)
    ...
    # TODO make compatible with our action head
    ...
    def _predict(self, observation: th.Tensor, first, state_in, deterministic: bool = False) -> th.Tensor:
        ...
        """
        (pi_logits, vpred, _), state_out = self.minerl_agent.policy(observation, first, state_in)
        action = self.minerl_agent.policy.pi_head.sample(pi_logits, deterministic=deterministic)
    ...
    def get_distribution(self, obs: th.Tensor, first, state_in) -> Distribution:
        ...
        """
        (latent_pi, latent_vf), state_out = self.minerl_agent.policy.net(obs, first, state_in)
