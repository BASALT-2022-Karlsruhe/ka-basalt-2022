                            ...
                            for kl_loss_weight in kl_loss_weights:
                                for max_batch in max_batches:
                                    ...
                                    with open(".env", "w+") as f:
                                        f.write(
                                            f"NAME=kulbach_{max_batch}_{n_worker}_{learning_rate}_{weight_decay}_{kl_loss_weight}\n"
                                        )
                                        f.write("VERSION='0_0_1'\n")
                                        f.write(
                                            "MODELS_ROOT='/home/shared/BASALT/models'\n"
                                        )
                                        f.write("PORT=9898\n")
                                        f.write("PYTHONUNBUFFERED=1\n")
                                        f.write("EVALUATION_STAGE='all'\n")
                                        f.write(
                                            "GIT_ACCESS_TOKEN=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\n"
                                        )
